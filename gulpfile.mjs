...
function serve(done) {
  server.init({
    server: './dist',
    port: 3000,
    files: ['dist/**/*.{html,css,js,data,png,jpg,jpeg,svg}'],
    notify: false,
    reloadDelay: 500,
    reloadDebounce: 1000
  });
  ...
  watch(paths.data, gulp.series(data, (done)  => {
    console.log('📁 JSON изменён, перезагрузка...');
    server.reload();
    done();
  })).on('error', (err) => console.error('Ошибка watch (JSON):', err));

  done();
}

// Задачи
const dev = series(clean, parallel(styles, data, scripts, images, html), serve);
const build = series(parallel(styles, data, scripts, images, html));
